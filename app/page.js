...
import { loadUserData, saveSnapshot, addScanFeedback } from '../lib/supabase/data';
...
// --- ICONS (SVG) ---
// Icon SVG đã tách sang app/_components/icons.js.
...
// --- MAIN APP EXPORT ---
// Ô nhập có icon trái + nút hiện/ẩn mật khẩu (cho màn auth).
...
function AppInner() {
    ...
    const handlePhoneAuth = async () => {
        ...
        if (authMode === "register" && pwd !== authConfirmPass.trim()) { toast.error("Mật khẩu xác nhận không khớp!"); return; }
   ...
   const removeFood = async (id) => {
                ...
                <div className="relative z-10 w-full max-w-md overflow-hidden rounded-[2rem] bg-surface shadow-[0_30px_80px_-20px_rgba(45,38,32,0.28)] ring-1 ring-cream-deep md:grid md:max-w-4xl md:grid-cols-2">
                    ...
                    <div className="p-7 sm:p-9">
                        ...
                        {isSupabaseConfigured && (
                            <>
                                <button onClick={handleGoogleLogin} disabled={loginLoading} className="mt-5 flex w-full items-center justify-center gap-2.5 rounded-2xl bg-cream-soft py-3.5 text-[13px] font-bold text-ink ring-1 ring-cream-deep transition hover:bg-cream-deep active:scale-[0.98] disabled:opacity-50">
                        ...
                        <div className="space-y-3.5">
                            ...
                            <AuthField label="Mật khẩu" icon={<IconLock />} reveal={{ shown: showPass, toggle: () => setShowPass(s => !s) }}>
                                <input type={showPass ? "text" : "password"} value={phonePass} onChange={e => setPhonePass(e.target.value)} placeholder="Tối thiểu 6 ký tự" className={inpPw} onKeyDown={e => { if (e.key === "Enter" && authMode === "login") handlePhoneAuth(); }} />
                            </AuthField>
                            {authMode === "register" && (
                                <AuthField label="Xác nhận mật khẩu" icon={<IconLock />} reveal={{ shown: showConfirm, toggle: () => setShowConfirm(s => !s) }}>
                                    <input type={showConfirm ? "text" : "password"} value={authConfirmPass} onChange={e => setAuthConfirmPass(e.target.value)} placeholder="Nhập lại mật khẩu" className={inpPw} onKeyDown={e => { if (e.key === "Enter") handlePhoneAuth(); }} />
                                </AuthField>
                            )}
                        </div>

                        <button onClick={handlePhoneAuth} disabled={loginLoading} className="mt-6 w-full rounded-2xl bg-orange py-4 text-[14px] font-black uppercase tracking-widest text-onaccent shadow-[0_12px_24px_-8px_rgba(217,119,87,0.6)] transition active:scale-[0.98] disabled:opacity-50 dark:shadow-[0_12px_30px_-8px_rgba(34,197,94,0.5)]">
                            {loginLoading ? "Đang kết nối..." : authMode === "register" ? "Tạo tài khoản" : "Đăng nhập"}
                        </button>
    ...
    if (showOnboarding) {
        const suggestedNick = authName.trim() || (displayName || "").split("@")[0] || "";
