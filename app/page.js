...
function AppInner() {
    ...
    const [signupName, setSignupName] = useState(""); // Họ tên khi đăng ký → gợi ý nickname onboarding
    ...
    const handleAuth = async (mode, { phone: rawPhone, pwd: rawPwd, confirm, name }) => {
        const phone = normPhone(rawPhone); const pwd = (rawPwd || "").trim();
        ...
        if (mode === "register" && pwd !== (confirm || "").trim()) { toast.error("Mật khẩu xác nhận không khớp!"); return; }
        if (mode === "register") setSignupName(name || "");
        ...
        try {
            ...
            if (mode === "login") {
                ...
                if (exists) { toast.error("Số điện thoại này đã có tài khoản — hãy Đăng nhập."); return 'exists'; }
   ...
   const removeFood = async (id) => {
    ...
    if (!userId || !password) {
        return <AuthScreen onAuth={handleAuth} onGoogle={handleGoogleLogin} loading={loginLoading} supabaseConfigured={isSupabaseConfigured} />;
    }

    // Lần đầu đăng nhập → onboarding wizard (đè lên mọi view).
    if (showOnboarding) {
        const suggestedNick = signupName.trim() || (displayName || "").split("@")[0] || "";
