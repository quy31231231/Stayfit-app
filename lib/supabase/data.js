...
export async function loadUserData() {
  ...
  if (!sb) { console.warn('[loadUserData] no supabase client'); return null; }
  const { data: ud, error: ue } = await sb.auth.getUser();
  console.log('[loadUserData] getUser →', ud?.user?.id, ud?.user?.email, '| err:', ue?.message);
  const user = ud?.user;
  if (!user) return null;
  const uid = user.id;
  ...
  console.log('[loadUserData] food:', (foods.data||[]).length, '| weight:', (weights.data||[]).length,
    '| custom:', (customs.data||[]).length, '| profile:', !!prof.data,
    '| errs:', foods.error?.message, weights.error?.message, prof.error?.message);
