...
export async function saveSnapshot(uid, { profile, history, weightLog, customFoods, deletedCommonFoods }) {
  ...
  await reconcileCustomFoods(sb, uid, cNames);
}

// Xóa custom_foods không còn trong state — fetch tên hiện có rồi xóa từng tên (parameterized).
// An toàn với mọi ký tự đặc biệt trong tên (vd dấu "), tránh tự ghép chuỗi `in (...)` dễ mất dữ liệu.
async function reconcileCustomFoods(sb, uid, keepNames) {
  const { data } = await sb.from('custom_foods').select('name').eq('user_id', uid);
  const keep = new Set(keepNames);
  const toDelete = (data || []).map((r) => r.name).filter((n) => !keep.has(n));
  for (const name of toDelete) {
    await sb.from('custom_foods').delete().eq('user_id', uid).eq('name', name);
  }
}
